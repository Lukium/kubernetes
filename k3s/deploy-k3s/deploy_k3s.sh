...
unset nodes masters agents storage workers k3sVersion cert_manager_version install_rancher rancher_hostname rancher_bootstrap_password install_longhorn install_traefik traefik_ip traefik_username traefik_password traefik_domain traefik_cf_token traefik_secret traefik_letsencrypt_email traefik_cloudflare_email install_pihole pihole_volume_capacity pihole_webpassword pihole_timezone pihole_ip use_ssh_passphrase copy_ssh_id cert_name no_SHKC after_SHKC keep_manifestes user interface vip lbrange ssh_agent_output SSH_AGENT_PID traefik_username_hash expose_rancher
